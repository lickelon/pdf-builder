...
class AnswerBuilder:
    ...
    def bake_unit_cover(self, unit_code, num):
        ...
        unit_cover = AreaOverlayObject(0, Coord(0,0,0), Ratio.mm_to_px(30))
        unit_cover.add_child(ComponentOverlayObject(0, Coord(0,0,0), component))
        unit_cover.add_child(TextOverlayObject(0, Coord(Ratio.mm_to_px(49), Ratio.mm_to_px(6), 1), "resources/fonts/Pretendard-Bold.ttf", 13, f"{num}. {self.get_unit_title(unit_code)}", tuple([int(num%3 == 1)]*3), fitz.TEXT_ALIGN_CENTER))
        return unit_cover
    ...
    def build(self):
        ...
        self.resources_pdf = "resources/weekly_ans_resources.pdf"
        ...
        for topic_set in self.items:
            for item in topic_set[1]:
                item_pdf = self.problem_dict[item["item_code"]]
