...
#     item_list = []
#     for item in items:
#         if item['mainsub'] == 'main':
#             item_list.append(item)
    
#     return item_list

# def bake_origin(source):
...
# def build_right(resources_pdf, resources_doc, item_code, page_num):
#     right_page = AreaOverlayObject(page_num, Coord(0,0,0), Ratio.mm_to_px(371))
#     origin = bake_origin(code_to_text(item_code))
#     item_pdf = f"KiceDB/{item_code[2:5]}/{item_code}/{item_code}_original.pdf"
...
#     right_page.add_child(TextOverlayObject(0, Coord(Ratio.mm_to_px(108), Ratio.mm_to_px(59), 0), "resources/fonts/Montserrat-Bold.ttf", 48, "A", (0.75, 0.4, 0, 0), fitz.TEXT_ALIGN_RIGHT))
...
#     print(y_end)
#     if y_end < 180: memo_component = Component(resources_pdf, 39, resources_doc.load_page(39).rect)
#     elif y_end < 197.5: memo_component = Component(resources_pdf, 40, resources_doc.load_page(40).rect)
#     elif y_end < 211: memo_component = Component(resources_pdf, 41, resources_doc.load_page(41).rect)
#     elif y_end < 238: memo_component = Component(resources_pdf, 42, resources_doc.load_page(42).rect)
#     elif y_end < 278.5: memo_component = Component(resources_pdf, 43, resources_doc.load_page(43).rect)
...
#     OX_component = None
#     if TF == 1:
#         OX_component = Component(resources_pdf, 13, resources_doc.load_page(13).rect)
#     if TF == 0:
#         OX_component = Component(resources_pdf, 14, resources_doc.load_page(14).rect)
...
# def build_left(resources_pdf, resources_doc, item_code, page_num):
#     left_page = AreaOverlayObject(page_num, Coord(0,0,0), Ratio.mm_to_px(371))
#     left_page.add_child(TextOverlayObject(0, Coord(Ratio.mm_to_px(18), Ratio.mm_to_px(59), 0), "resources/fonts/Montserrat-Bold.ttf", 48, "A", (0.75, 0.4, 0, 0), fitz.TEXT_ALIGN_LEFT))
#     origin = bake_origin(code_to_text(item_code))
...
#     main_pdf = f"input/weekly2/KiceDB/{item_code[2:5]}/{item_code}/{item_code}_Main.pdf"
...
#         sol_commentary_data = get_commentary_data()[solution_info.hexcode]
...
#             fc_bar_component = Component(resources_pdf, 37, resources_doc.load_page(37).rect)
...
#             continue
#         so = bake_solution_object(resources_pdf, resources_doc, solution_info, sTF[solution_info.hexcode], main_pdf)
...
#     solving_bar_component = Component(resources_pdf, 38, resources_doc.load_page(38).rect)
...
#         overlayer.add_page(Component(resources_pdf, 12, resources_doc.load_page(12).rect))
#         right_page = build_right(resources_pdf, resources_doc, item_list[i]['item_code'], 0)
#         right_page.overlay(overlayer, Coord(0,0,0))
#         overlayer.add_page(Component(resources_pdf, 11, resources_doc.load_page(11).rect))
#         left_page = build_left(resources_pdf, resources_doc, item_list[i]['item_code'], 1)    
#         left_page.overlay(overlayer, Coord(0,0,0))

#     new_doc.save("output/weekly_main_test.pdf")
#     resources_doc.close()
...
class MainsolBuilder:
    ...
    def build_right(self, item_code, page_num):
        right_page = AreaOverlayObject(page_num, Coord(0,0,0), Ratio.mm_to_px(371))
        origin = self.bake_origin(self.code_to_text(item_code))
        item_pdf = f"KiceDB/{item_code[2:5]}/{item_code}/{item_code}_original.pdf"
        with fitz.open(item_pdf) as file:
            page = file.load_page(0)
            component = Component(item_pdf, 0, page.rect)
