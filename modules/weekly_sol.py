...
class SolutionBuilder:
    ...
    def bake_unit_cover(self, unit_code, num):
        ...
        unit_cover.add_child(ComponentOverlayObject(0, Coord(0,0,0), component))
        unit_cover.add_child(TextOverlayObject(0, Coord(Ratio.mm_to_px(7.5), Ratio.mm_to_px(10), 1), "resources/fonts/Cafe24Ohsquare-v2.0.ttf", 25, f"{num}", (1,1,1), fitz.TEXT_ALIGN_CENTER))
        unit_cover.add_child(TextOverlayObject(0, Coord(Ratio.mm_to_px(20), Ratio.mm_to_px(10), 1), "resources/fonts/Pretendard-SemiBold.ttf", 16.5, f"{self.get_unit_title(unit_code)}", (0,0,0), fitz.TEXT_ALIGN_LEFT))
    ...
    def build(self):
        ...
        #setup
        self.problem_dict = get_problem_dict("input/weekly2")
        sol_type_dict = self.get_sol_type_dict()

        with open("resources/commentary.json") as file:
            commentary_data = json.load(file)

        self.resources_pdf = "resources/weekly_sol_resources.pdf"
        ...
        for topic_set in self.items:
            ...
            for item in topic_set[1]:
                problem_objects = self.bake_problem(commentary_data, self.problem_dict[item["item_code"]], item["item_num"])
                for problem_object in problem_objects:
